...
IS_SQLITE = not (DB_URL.startswith("postgres://") or DB_URL.startswith("postgresql://"))
SQLITE_DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hiremind.db")

class SQLiteRow(dict):
    """Dict-like row object supporting key-name access and integer index access."""
...
class SQLiteCursorWrapper:
    ...
    def _transform_query(self, query: str, params):
        q = query
        ...
        if p is not None and re.search(r'=\s*ANY\s*\(\s*%s\s*\)', q, flags=re.IGNORECASE):
            new_params = []
            param_idx = 0
            def any_replacer(match):
                nonlocal param_idx
                if param_idx < len(p):
                    val = p[param_idx]
                    param_idx += 1
            ...
            q = re.sub(r'=\s*ANY\s*\(\s*%s\s*\)', any_replacer, q, flags=re.IGNORECASE)
            while param_idx < len(p):
                new_params.append(p[param_idx])
                param_idx += 1
            p = new_params
        
        # Strip PostgreSQL type casts like ::json, ::text, etc.
        q = re.sub(r'::\w+', '', q)
    ...
    def execute(self, query: str, params=None):
        transformed_query, transformed_params = self._transform_query(query, params)
        if transformed_params is not None:
            return self.cursor.execute(transformed_query, tuple(transformed_params))
        return self.cursor.execute(transformed_query)

    def executemany(self, query: str, seq_of_parameters):
        transformed_query, _ = self._transform_query(query, None)
        return self.cursor.executemany(transformed_query, seq_of_parameters)

    def fetchone(self):
        row = self.cursor.fetchone()
        if row is None:
            return None
        return SQLiteRow(self.cursor, row)

    def fetchall(self):
        rows = self.cursor.fetchall()
        return [SQLiteRow(self.cursor, r) for r in rows]
    ...
    def __iter__(self):
        for row in self.cursor:
            yield SQLiteRow(self.cursor, row)
...
class SQLiteConnectionWrapper:
    ...
    def cursor(self, cursor_factory=None):
        return SQLiteCursorWrapper(self._conn.cursor(), conn=self._conn)
...
def get_db_connection():
    ...
    if psycopg2 is None:
        raise Exception("psycopg2 is required for PostgreSQL connections.")
